...
class TabNAT(nn.Module):
    def __init__(self, n_num, n_cat, categories, embed_dim=32, buffer_size=8, depth=6, norm_layer=nn.LayerNorm, dropout_rate=0.0, device='cuda:0'):
        ...
        # Conditional generation (dummy embedding if no categorical columns;
        # unused when cls=None is passed at forward / sample time)
        self.class_embedding = nn.Embedding(categories[0] if len(categories) > 0 else 1, embed_dim)
        ...
        # Loss
        # dim_t = ddiff (paper Appendix B.4 Table 7: 512). Code release had 1024 hardcoded.
        self.mixed_loss = Mixed_Loss(n_num, categories, embed_dim, dim_t=512, dropout_rate=dropout_rate)
