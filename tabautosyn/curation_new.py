...
class VerifiedConfidenceFilter:
    ...
    def curate(
        ...
        cutoff = np.quantile(confidence, self.drop_quantile)
        ...
        if score_after >= score_before:
            print(
                f"  [ConfFilter] {len(syn_data)} -> {len(candidate)} rows, "
                f"score: {score_before:.4f} -> {score_after:.4f} (applied)"
            )
            return candidate

        print(
            f"  [ConfFilter] Would drop score {score_before:.4f} -> "
            f"{score_after:.4f}, ROLLED BACK"
        )
        return syn_data
...
class BackwardElimination:
    ...
    def __init__(
        ...
        self.min_rows_ratio = min_rows_ratio
    ...
    def _generate_removals(
        ...
        min_rows = max(_MIN_ACTIVE_ROWS, int(len(active_idx) * self.min_rows_ratio))
    ...
    def curate(
        ...
        while no_improve < self.patience:
            round_num += 1
            active_idx = np.where(active_mask)[0]
            if len(active_idx) < max(_MIN_ACTIVE_ROWS, int(n * self.min_rows_ratio)):
                break
            ...
            if best_removal is not None:
                ...
                print(
                    f"  [BackwardElim round {round_num}] "
                    f"Removed {len(best_removal)} rows, score: {current_score:.4f} (+{delta:.4f})"
                )
                no_improve = 0
            else:
                no_improve += 1
        ...
        current_score = self._forward_recovery(
            removed_history, active_mask, X, y, Xr, yr, n_classes, scoring_fn, current_score, task,
        )
    ...
    def _forward_recovery(
        self, history, active_mask, X, y, Xr, yr, n_classes, scoring_fn, current_score, task,
    ) -> float:
        if len(history) <= 1:
        ...
        for i, block in enumerate(history):
            ...
            if score > current_score:
                ...
                current_score = score
                print(
                    f"  [ForwardRecovery] Restored block {i} "
                    f"({len(block)} rows), score: {current_score:.4f}"
                )
        return current_score
...
class TargetedRowRefinement:
    ...
    def curate(
        ...
        for pass_num in range(self.max_passes):
            active_idx = np.where(active_mask)[0]
            if len(active_idx) < _MIN_ACTIVE_ROWS:
                break
            ...
            if use_fast:
                pass_removed, current_score = self._singles_fast(
                    ...
                    scoring_fn, fast_scoring_fn, max_singles, patience, task,
                )
            else:
                pass_removed, current_score = self._singles_full(
                    candidates, active_mask, X, y, Xr, yr,
                    scoring_fn, max_singles, patience, task,
                )

            pass_removed += self._pairs(
                active_mask, X, y, Xr, yr, scoring_fn, n_classes,
                max_groups, patience, pass_num, task,
            )[0]
            current_score = scoring_fn(X[active_mask], y[active_mask], Xr, yr)
            ...
            if pass_removed == 0:
                break
            print(
                f"  [TargetedRefine pass {pass_num + 1}] "
                f"Removed {pass_removed} rows, score: {current_score:.4f}"
            )

        if total_removed > 0:
            print(f"  [TargetedRefine] Total removed: {total_removed} rows")

        return syn_data.iloc[np.where(active_mask)[0]].reset_index(drop=True)

    # -- single-row removal --

    def _singles_fast(self, candidates, active_mask, X, y, Xr, yr, scoring_fn, fast_fn, max_singles, patience, task):
    ...
    def _singles_full(self, candidates, active_mask, X, y, Xr, yr, scoring_fn, max_singles, patience, task):
        current_score = scoring_fn(X[active_mask], y[active_mask], Xr, yr)
        no_improve = 0
        tried = 0
        removed = 0
    ...
    def _pairs(self, active_mask, X, y, Xr, yr, scoring_fn, n_classes, max_groups, patience, pass_num, task):
        active_idx = np.where(active_mask)[0]
...
class CuratorConfig:
    ...
    Args:
        ...
        min_rows_ratio: Never drop below this fraction of the original data.
...
class SyntheticDataCurator:
    ...
    def curate(
        ...
        self._log(f"[Curator] Baseline: {initial_size} rows, score={baseline:.4f}")

        current = self._run_confidence_filter(current, real_data, feature_cols, target_col)
        current = self._run_backward_elimination(current, real_data, feature_cols, target_col)
        current = self._run_row_refinement(current, real_data, feature_cols, target_col)
    ...
    def _run_confidence_filter(self, current, real_data, feature_cols, target_col):
        if self.config.confidence_drop <= 0:
            return current
        cf = VerifiedConfidenceFilter(drop_quantile=self.config.confidence_drop)
        current = cf.curate(current, real_data, feature_cols, target_col, self.scoring_fn, self.task)
        self._log_stage("ConfidenceFilter", current, real_data, feature_cols, target_col)
        return current

    def _run_backward_elimination(self, current, real_data, feature_cols, target_col):
        be = BackwardElimination(
            ...
            min_rows_ratio=self.config.min_rows_ratio,
        )
        current = be.curate(current, real_data, feature_cols, target_col, self.scoring_fn, self.fast_scoring_fn, self.task)
        self._log_stage("BackwardElim", current, real_data, feature_cols, target_col)
        return current

    def _run_row_refinement(self, current, real_data, feature_cols, target_col):
        refiner = TargetedRowRefinement(max_passes=self.config.refine_passes)
        current = refiner.curate(current, real_data, feature_cols, target_col, self.scoring_fn, self.fast_scoring_fn, self.task)
        return current
    ...
    def _log_stage(self, name: str, current, real_data, feature_cols, target_col) -> None:
        ...
        print(f"[Curator] After {name}: {len(current)} rows, score={s:.4f}")
    ...
    def _log_final(self, current, real_data, feature_cols, target_col, initial_size, baseline) -> None:
        ...
        print(
            f"[Curator] Final: {len(current)}/{initial_size} rows "
            f"({100 * len(current) / initial_size:.1f}%), "
            f"score={final:.4f} (delta={final - baseline:+.4f})"
        )
...
def curate_synthetic_data(
    ...
    """
    config = CuratorConfig(task=task, scoring_fn=scoring_fn, verbose=verbose)
