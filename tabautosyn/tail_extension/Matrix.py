    ...
    for i in range(n):
        for j in range(n):
            ...
            if metric == "mi":
                ...
                p, _ = np.histogram(
                    x, bins=n_bins, range=(np.min(x), np.max(x)), density=True
                )
                q, _ = np.histogram(
                    y, bins=n_bins, range=(np.min(y), np.max(y)), density=True
                )
                val = js_divergence(p + 1e-12, q + 1e-12)

            else:
                raise ValueError(
                    "Неизвестная метрика! Используй 'mi', 'wasserstein' или 'js'."
                )
...
def matrix_cosine_similarity(real_df, synth_df, metric="mi", show_plots=False):
    ...
    for i in range(mat_real.shape[0]):
        cos_sim = cosine_similarity(
            mat_real[i].reshape(1, -1), mat_synth[i].reshape(1, -1)
        )[0, 0]
    ...
    if show_plots:
        plot_cosine_comparison(
            mat_real, mat_synth, cosine_score, flattened_cosine, metric
        )
...
def matrix_frobenius_distance(real_df, synth_df, metric="mi", show_plots=False):
    ...
    if show_plots:
        plot_frobenius_comparison(
            mat_real, mat_synth, frobenius_dist, normalized_frobenius, metric
        )
...
def plot_cosine_comparison(mat_real, mat_synth, cosine_score, flattened_cosine, metric):
    ...
    fig.suptitle(
        f"Косинусное сходство (построчное): {cosine_score:.4f}\n"
...
def plot_frobenius_comparison(
    ...
    fig.suptitle(
        f"Расстояние Фробениуса: {frobenius_dist:.4f}\n"
