...
class GeneticAlgorithm:
    ...
    def _evaluate_population(self, population: List[Individual], test_data: pd.DataFrame):
    ...
    def _evolve_generation(self, population: List[Individual]) -> List[Individual]:
        ...
        offspring = [Individual(list(ind.data), ind.feature_cols, ind.target_col) 
    ...
    def run(self, 
            ...
            initial_population: Optional[List[pd.DataFrame]] = None) -> List[GAResult]:
        ...
        if self.global_pool is None:
            self.global_pool = create_global_pool(initial_population, feature_cols, self.target_col)
        ...
        population = [Individual.from_dataframe(df, feature_cols, self.target_col) 
        ...
        for gen in range(1, self.config.n_generations + 1):
            ...
            if self.config.verbose:
                print(f"Gen {gen}: max={self.history['max'][-1]:.4f} "
...
def bootstrap_sample(data: pd.DataFrame, n_samples: int = 1000, 
                     sample_ratio: float = 0.7) -> List[pd.DataFrame]:
...
def create_global_pool(dataframes: List[pd.DataFrame], 
                      feature_cols: List[str], 
                      target_col: str) -> List[Tuple]:
